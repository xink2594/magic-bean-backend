...
- **图片存储**: AWS S3 SDK 对接 Cloudflare R2。
- **AI 视觉**: 阿里云 DashScope SDK（`qwen3.5-flash` 模型）。
...
数据写入由 EMQX 规则引擎完成（MQTT → MySQL），后端只负责读取和 diary 的增删改。
...
| GET/POST | `/api/diary/**` | 手记 CRUD + 回收站 |
